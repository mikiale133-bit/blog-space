    ...
    <div>
      ...
      <div className="mt-8 card">
        <header className="flex items-center justify-between mb-5">
          <h2 className="mb-4 text-xl font-bold">ASSESSMENTS</h2>
          <button className="btn btn-primary">Add Assignment</button>
        </header>

        {assessments.length === 0 ? (
          <div className="card">
            <div className="flex flex-col items-center justify-center p-5 text-gray-500 gap-">
              <BookOpen size={40} className="" />
              <h2 className="text-lg italic font-semibold">No quizzes yet !</h2>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            {assessments.map((ass) => (
              <Link to={`/student-dashboard/assessments/${ass._id}`} key={ass._id} className="flex items-center justify-between p-4 rounded bg-muted">
                <div>
