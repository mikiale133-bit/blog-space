...
const MyClass = () => {
  ...
  useEffect(() => {
    const getGroups = async () => {
      try {
        const res = await API.get(`/api/classes/${id}/groups/${subjectId}`);
  ...
  useEffect(() => {
    try {
      const getQuizzes = async () => {
        const res = await API.get(`/api/classes/${id}/quizzes/${subjectId}`);
  ...
  useEffect(() => {
    try {
      const getQuizzes = async () => {
        const res = await API.get(`/api/classes/${id}/assessments/${subjectId}`);
  ...
  const createGroup = async () => {
    ...
    try {
      await API.post(`/api/classes/${id}/groups/${subjectId}`, {
        groupName: groupName,
  ...
  // SELECTION MODE FOR GROUP MODAL
  if (selectionMode) {
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="card">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 pt-4 border-t border-border">
            <button
              onClick={selectionModeType === "create-group" ? createGroup : addStudentToGroup}
  ...
  if (quizModal) {
    return <QuizForm classId={classRoom._id} subjectId={subjectId} />;
  }

  if (assessmentMdal) {
    return <AssessmentForm classId={classRoom._id} subjectId={subjectId} groupId={activeGroupId} />;
  }

  if (uploadModal) {
    return <UploadForm classId={classRoom._id} subjectId={subjectId} groupId={activeGroupId} />;
  }
  ...
  return (
    <div className="">
      {/* Class Header */}
      <div className="mb-6">
        {/* <h1 className="text-3xl font-bold text-foreground mb-2">{classRoom.subjects.filter(sub => sub === )}</h1> */}
        <div className="flex flex-col flex-wrap gap-1 mb-3 text-sm text-foreground/70">
          <span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className={`${cardStyles} hover:shadow-lg transition-shadow`}>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {groups.length === 0 ? (
            <div className="col-span-full p-3 text-center py-12 text-foreground/60">
